...
var (
	createMessage1 = &pb.CreateObjectRequest{
		Key: &pb.Key{
			Parts: []*pb.Key_Part{
	...
	createMessage2 = &pb.CreateObjectRequest{
		Key: &pb.Key{
			Parts: []*pb.Key_Part{
	...
	CreateMessage3 = &pb.CreateObjectRequest{
		Key: &pb.Key{
			Parts: []*pb.Key_Part{
...
func TestServer_PostObject_GetObject(t *testing.T) {
	/*
		cases := []struct {
			...
			{newServer(), createMessage1, "foo=123~bar=456", "post message 1"},
			{newServer(), createMessage2, "foo=123~bar=654", "post message 2"},
			{newServer(), CreateMessage3, "foo=321~bar=456", "post message 3"},
		}

		for i, c := range cases {
			expectedResp := createPostMessageResponse(c.name)
			if resp, _ := c.s.CreateObject(nil, c.req); !reflect.DeepEqual(resp, expectedResp) {
				t.Errorf("case %v failed", i)
			}

			resp2, _ := c.s.GetObject(nil, createGetObjectRequest(c.name))
			if data := string(resp2.GetEntries()[0].GetData()); data != c.data {
...
func TestServer_Indexing(t *testing.T) {
	s := newServer()
	_, _ = s.CreateObject(nil, createMessage1)
	_, _ = s.CreateObject(nil, createMessage2)
	_, _ = s.CreateObject(nil, CreateMessage3)
	...
	for i, c := range cases {
		...
		if c.fail {
			...
			if len(resp.GetEntries()) != len(c.dataz) {
				t.Errorf("case %v: response count mismatch %v <> %v", i, len(resp.GetEntries()), len(c.dataz))
			}
